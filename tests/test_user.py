...
class TestUser(unittest.TestCase):
    ...
    def test_login(self, mock_find_one):
        ...
        response_valid_credentials = self.app.post('/login', json={'email': 'test@test.com', 'password': 'test_password'})
        self.assertEqual(response_valid_credentials.status_code, 200)
    
        response_invalid_credentials = self.app.post('/login', json={'email': 'test@test.com', 'password': 'wrong_password'})
